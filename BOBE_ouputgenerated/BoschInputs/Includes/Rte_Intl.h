...
#if defined(RTE_OSAPI_AUTOSAR_R40)

/* [$Satisfies $rte_swaddr 097] */
#define RTE_START_SEC_VAR_CLEARED_16
#include "Rte_MemMap.h"
extern VAR(uint16, RTE_DATA) Rte_SpinlockCount[RTE_MAX_USED_CORE_ID + 1];
...
#ifndef Rte_GetSpinlock
# define Rte_GetSpinlock() { CoreIdType coreId; Rte_SuspendOSInterrupts(); coreId = GetCoreID(); if (!Rte_SpinlockCount[coreId]) { (void)GetSpinlock(RTE_SPINLOCK); } ++Rte_SpinlockCount[coreId]; }
#endif

#ifndef Rte_ReleaseSpinlock
# define Rte_ReleaseSpinlock() { if ( 0u == --Rte_SpinlockCount[GetCoreID()] ) { (void)ReleaseSpinlock(RTE_SPINLOCK); } Rte_ResumeOSInterrupts(); }
#endif
...
extern VAR(boolean, RTE_DATA) Rte_Initializing;
